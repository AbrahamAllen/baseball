...
class Unit{
	constructor(){
		...
		let a = this;
		this.active = setInterval(function(){a.animate()}, 10);
	}
...
class Pitcher extends Unit{
	...
	input(key){
		switch(key.key){
			case 'a' : this.dx = -2; break;
			case 'd' : this.dx = 2;break;
...
class PitchedBall extends Unit{
	...
	pitched(){
		...
		let me = this;
		this.clear = setTimeout(function(){me.reset()}, 2000);
	}
...
class HitBall extends Unit{
	constructor(bat, ball){
		...
		this.interval = setInterval(function(){
			...
			me.dom.dom.style.height = me.z+'px'}, 10);
		
		this.setD(bat, ball);
		this.friction = setTimeout(function(){me.dx = 0; me.dy = -0.3}, 4000)
		
	}
	
	setD(bat, ball){
		this.dy = Math.ceil(bat.c/(-2));
		this.max = 40;
	...
	makeSpot(){
		let dx = this.dx*4*(this.max-15);
		let dy = this.dy*4*(this.max-15);
...
class RunnerTracker{
	...
	build(){
		...
		this.interval = setInterval(this.set, 10);
		off.makeminis();
	}
